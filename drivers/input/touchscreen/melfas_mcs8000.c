...
#define NEW_FIRMWARE_VERSION 	 0x15
...
void firmware_update()
{
	...
	else if((melfas_mcs8000_ts->window_ver==MELFAS_WINDOW_70T)&& (melfas_mcs8000_ts->hw_rev == 0x09))
    {	
	...
	else if((melfas_mcs8000_ts->window_ver==MELFAS_WINDOW_70T) && (melfas_mcs8000_ts->hw_rev == 0x02))
    {	
...
#endif
/*          if (keyID == 0x1)
                input_report_key(melfas_mcs8000_ts->input_dev, KEY_MENU, keyState ? PRESS_KEY : RELEASE_KEY);
            if (keyID == 0x2)
                input_report_key(melfas_mcs8000_ts->input_dev, KEY_HOME, keyState ? PRESS_KEY : RELEASE_KEY);
            if (keyID == 0x3)
                input_report_key(melfas_mcs8000_ts->input_dev, KEY_BACK, keyState ? PRESS_KEY : RELEASE_KEY);
            if (keyID == 0x4)
                input_report_key(melfas_mcs8000_ts->input_dev, KEY_SEARCH, keyState ? PRESS_KEY : RELEASE_KEY);*/
#if DEBUG_PRINT
               ...
               const struct i2c_device_id *id)
{
    ...
    {	
        if((melfas_mcs8000_ts->hw_rev == 0x09)&&(board_hw_revision >= 4) && (melfas_mcs8000_ts->fw_ver < NEW_FIRMWARE_VERSION) && (version_read == TRUE))
        {
            printk("[TSP] firmware update window 0.7T\n");
            firmware_update();
        }
	...
	if(melfas_mcs8000_ts->fw_ver < 0x02)
