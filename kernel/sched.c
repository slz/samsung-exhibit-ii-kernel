...
#define ALEX_TASK
#define ALEX_TASK_VER (11.8.19)
#endif
#ifdef ALEX_TASK
#define MAX_KERNEL_SCHEDULE_LOGS	(1000)
...
static struct ScheduleLogData pScheduleLogData[MAX_KERNEL_SCHEDULE_LOGS];
static int nNextLogIdx = 0;
#endif
	       ...
	       struct task_struct *next)
{
...
#ifdef ALEX_TASK
	{
		...
		if (prev != NULL) {
			...
			pScheduleLogData[nNextLogIdx].unPid = prev->pid;
			pScheduleLogData[nNextLogIdx].cpu = smp_processor_id();
			strncpy(pScheduleLogData[nNextLogIdx].ucComm, prev->comm, 16);

			nNextLogIdx++;
			if( nNextLogIdx == MAX_KERNEL_SCHEDULE_LOGS )
				nNextLogIdx = 0;

			pScheduleLogData[nNextLogIdx].ulTick = 0xFEED42AD;
		}
	}
#endif
